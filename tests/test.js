...
T('층 58개 (…+리부트7+무한루프8+클라우드8+소스8)', () => FLOORS.length === 58);
...
T('보스 58명, 파티 종족 존재', () => BOSSES.length === 58 && BOSSES.every(B => B.party.every(([sid]) => !!SPECIES[sid])));
...
T('스토리 60개·진행 라벨 60개 (8부까지)', () => STORY.length === 60 && PROG_LABEL.length === 60);
T('최종 스토리 인덱스 범위 내 (4부 진엔딩)', () => { G.story = 27; G.isekai = true; const i = storyIdx(); G.story = 0; G.isekai = false; return i === 28 && i < STORY.length; });
T('8부 진진엔딩 스토리 인덱스 (원작자 격파)', () => { G.story = 58; G.isekai = true; const i = storyIdx(); G.story = 0; G.isekai = false; return i === 59 && i === STORY.length - 1; });
...
/* ── 전설 도감/랭킹 ── */
T('전설 종족 53종, 전부 입수처 등록', () => {
  const legends = ALL_SIDS.filter(s => SPECIES[s].legend);
  return legends.length === 53 && legends.every(s => !!LEGEND_ROUTE[s]);
});
...
T('차원 8개: 5~8부(need 27/34/42/50) 추가', () =>
  DIMENSIONS.length === 8 &&
  DIMENSIONS[4].from === 27 && DIMENSIONS[4].need === 27 && DIMENSIONS[4].isekai === true &&
  DIMENSIONS[5].from === 34 && DIMENSIONS[5].need === 34 &&
  DIMENSIONS[6].from === 42 && DIMENSIONS[6].to === 49 && DIMENSIONS[6].need === 42 && DIMENSIONS[6].isekai === true &&
  DIMENSIONS[7].from === 50 && DIMENSIONS[7].to === 57 && DIMENSIONS[7].need === 50 && DIMENSIONS[7].isekai === true);
...
T('공용: rarMark 등급 심볼 (일반은 빈 문자열)', () =>
  rarMark('gold') !== '' && rarMark('prism') !== '' && rarMark('legend') !== '' && rarMark('mythic') !== ''
  && rarMark('normal') === '' && rarMark('gold') !== rarMark('prism'));
...
T('밸런스: 6부 무한루프 보스 mult ≥ 1.7, 옴니루프 ≥ 2.0', () => {
  const six = BOSSES.slice(34, 42);
  return six.length === 8 && six.every(B => B.mult >= 1.7) && BOSSES[41].mult >= 2.0;
});
T('밸런스: 7·8부 mult 단조 상승 (6부<7부<8부, 원작자 최고 2.7)', () => {
  const six = BOSSES.slice(34, 42), seven = BOSSES.slice(42, 50), eight = BOSSES.slice(50, 58);
  return seven.every(B => B.mult >= 2.0) && eight.every(B => B.mult >= 2.3) && BOSSES[57].mult >= 2.7
    && Math.max.apply(null, six.map(B => B.mult)) <= Math.min.apply(null, seven.map(B => B.mult)) + 1e-9
    && Math.max.apply(null, seven.map(B => B.mult)) <= Math.min.apply(null, eight.map(B => B.mult)) + 1e-9;
});

/* ── v6.5 7·8부 (클라우드/소스) 확장 ── */
T('7·8부 신규 전설 앵커 16종 존재·legend', () =>
  ['p7aOrchestratus', 'p7aTensorking', 'p7aServerlexus', 'p7aHyperscaler', 'p7bAutomata', 'p7bOverfitgolem', 'p7bNeuralhydra', 'p7bSingularis',
    'p8aCompilatron', 'p8aLintseraph', 'p8aMergewraith', 'p8aCoveragus', 'p8bCompilus', 'p8bDebuggernaut', 'p8bMergemonarch', 'p8bTheauthor']
    .every(s => SPECIES[s] && SPECIES[s].legend === true));
T('진진엔딩 최종보스 원작자가 옴니루프보다 강하다', () =>
  SPECIES.p8bTheauthor.hp > SPECIES.lpbOmniloop.hp && SPECIES.p8bTheauthor.atk > SPECIES.lpbOmniloop.atk);
T('7·8부 합성 전용종 4종 fusion·레시피 존재', () =>
  ['p7aMeshmind', 'p7bSynthcore', 'p8aRefactitan', 'p8bRefactorrend'].every(r => {
    const rc = FUSION_RECIPES.find(x => x.to === r);
    return SPECIES[r] && SPECIES[r].fusion === true && rc && rc.mats.every(s => !!SPECIES[s]);
  }));
T('7·8부 늦은 진화: lv230/260 라인 존재', () => {
  const lv = s => SPECIES[s] && SPECIES[s].evo && SPECIES[s].evo.lv;
  return lv('p7bClockworm') === 230 && (lv('p8bRevertspore') === 260 || lv('p8bMergemaw') === 260);
});
T('8부 진진엔딩 화면 렌더 (원작자 격파)', () => {
  newGame(); G.party = [makeMon('espresso', 260)]; G.isekai = true; G.story = 58; G.screen = 'ending'; render(); return true;
});
...
/* ── v6.4 합성 재료 개체 선택 ── */
T('합성: doCrossFuse — 선택 개체 소멸·신종 생성·비용 차감·fuseSel 해제', () => {
  newGame(); G.money = 999999; G.box = [];
  G.party = [makeMon('latte', 27, 'normal'), makeMon('gian', 27, 'normal')];
  const rc = FUSION_RECIPES.find(r => r.to === 'cafegriffin');
  const a = G.party.find(m => m.sid === 'latte'), b = G.party.find(m => m.sid === 'gian');
  const m0 = G.money; doCrossFuse(rc, a, b);
  const all = [...G.party, ...G.box];
  return all.some(m => m.sid === 'cafegriffin') && !all.includes(a) && !all.includes(b)
    && G.money === m0 - RECIPE_COST && fuseSel === null;
});
T('합성: 결과 등급은 높은 쪽(골드) 이상 계승', () => {
  newGame(); G.money = 999999; G.box = [];
  G.party = [makeMon('latte', 27, 'gold'), makeMon('gian', 27, 'normal')];
  const rc = FUSION_RECIPES.find(r => r.to === 'cafegriffin');
  doCrossFuse(rc, G.party.find(m => m.sid === 'latte'), G.party.find(m => m.sid === 'gian'));
  const nm = [...G.party, ...G.box].find(m => m.sid === 'cafegriffin');
  return nm && RARITY[nm.rar].rank >= RARITY.gold.rank;
});
T('합성 재료 선택 화면 렌더 (다중 후보 → 자동배정 → 확인)', () => {
  newGame(); G.box = [];
  G.party = [makeMon('latte', 27, 'gold'), makeMon('latte', 27, 'normal'), makeMon('gian', 27, 'normal')];
  const rc = FUSION_RECIPES.find(r => r.to === 'cafegriffin');
  fuseSel = { rc, a: null, b: null }; G.screen = 'fusion'; render();
  fuseSel = { rc, a: G.party[0], b: null }; render();
  fuseSel = null; return true;
});
